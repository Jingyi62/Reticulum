...
func startNode(node *Node) {
	...
	for {
		...
		case newvote2 := <-node.CSchain.VoteQueue_send2:
			...
			n.PSBroadcastMessages <- NewMessage(node.ID, 2, new_vote_byte2)
			n.CSBroadcastMessages <- NewMessage(node.ID, 2, new_vote_byte2)
		case newVoteSummary := <-node.CSchain.VoteSummaryQueue_send:
			...
			if err != nil {
				log.Printf("error happened vote summary to bytes")
			}
...
func producePSBlocks(node *Node, n *Network, bc *PSchain) {
	...
	for {
		if node.PSchain.Len > sentBlockepoch { //当前len 从0开始，一旦len大于sent block epoch意味着上一个block已经被大家接受
