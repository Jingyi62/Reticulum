...
var PSchainlen int = 0
var CSchainlen int = 0
...
func SendAfter(delta int, node *Node, bc *PSchain, cc *CSchain) {
	...
	time.AfterFunc(time.Duration(delta)*time.Second, func() {
		...
		//node.VoteSummary = VoteSummary{node.VoteSummary.Origin, node.VoteSummary.CSindex, node.VoteSummary.Epoch, node.VoteSummary.AgreeMap, node.VoteSummary.Signature}
		...
		var temp []string
		cc.Vote4votesummary_vote2.Store(node.VoteSummary.Signature, votesummarystatus{1, temp, &node.VoteSummary})
		cc.Vote4votesummary_vote3.Store(node.VoteSummary.Signature, votesummarystatus{0, temp, &node.VoteSummary})
		VoteSummary_thisepoch = node.VoteSummary

	})
}
func UpdateVS(node *Node) {
	node.ifvotesummary = false
	node.VoteSummary.AgreeMap.Data = []KeyValuePair{}
	for j := 0; j < psincs; j++ {
		node.VoteSummary.AgreeMap.Data = append(node.VoteSummary.AgreeMap.Data, KeyValuePair{j, Votelist{}})
	}
	for i := 1; i < csnum+1; i++ {
		if node.VoteSummary.Epoch > int16(len(adv)) {
			node.VoteSummary.AgreeMap.Data[(i-1)/psnum].Value.Num += 1
			node.VoteSummary.AgreeMap.Data[(i-1)/psnum].Value.Nodelist = append(node.VoteSummary.AgreeMap.Data[(i-1)/psnum].Value.Nodelist, "node"+strconv.Itoa(i))
		} else if contains(advlist[(node.VoteSummary.Epoch)-1], i) {
		} else {
			node.VoteSummary.AgreeMap.Data[(i-1)/psnum].Value.Num += 1
			node.VoteSummary.AgreeMap.Data[(i-1)/psnum].Value.Nodelist = append(node.VoteSummary.AgreeMap.Data[(i-1)/psnum].Value.Nodelist, "node"+strconv.Itoa(i))
		}
