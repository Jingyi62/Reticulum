					...
					if stringInSlice(temp_vote.Origin, nowvote.Nodeid) {
						...
						if node.CSLeader == 1 && node.ifvotesummary == true && node.VotesummaryRound == bc.Len {
							if nowvotesummary, ok := node.VoteSummary.AgreeMap.Load(temp_vote.BlockHash); ok {
								...
								node.VoteSummary.AgreeMap.Store(temp_vote.BlockHash, VoteSummaryMap{nowvote.Num + 1, nowvote.Nodeid})
							}
						}
					}
				} else {
...
func (bc *PSchain) GenerateBlock(node *Node) {
	if nowvoteRaw, ok := node.VoteSummary.AgreeMap.Load(bc.MaybeNext.Hash); ok {
		Vote, ok := nowvoteRaw.(VoteSummaryMap)
		if !ok {
			return
		}
		if Vote.Num > (psnum - 2) {
