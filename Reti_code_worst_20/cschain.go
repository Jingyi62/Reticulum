...
func (cc *CSchain) Add(node *Node) {
	...
	err = saveElapsedTimeToFile(node, elapsed, "./data/"+cc.NodeID+"/csblock/"+strconv.Itoa(cc.Len)+"_time", cc.Len)
	if err != nil {
...
func saveElapsedTimeToFile(node *Node, elapsed time.Duration, filename string, epoch int) error {
	...
	defer file.Close()
	if epoch > 0 && epoch<5 {
		_, err = fmt.Fprintf(file, "Generate Block time: %d\n", 36+16, node.T2+T1)
	} else if epoch > 40 && epoch<45 {
		_, err = fmt.Fprintf(file, "Generate Block time: %d\n", 36+16, node.T2+T1)
	} else if epoch > 80 && epoch<85 {
		_, err = fmt.Fprintf(file, "Generate Block time: %d\n", 36+16, node.T2+T1)
	} else {
		_, err = fmt.Fprintf(file, "Generate Block time: %d\n", 9+16, node.T2+T1)
	}
